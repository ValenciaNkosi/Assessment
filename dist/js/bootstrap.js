    ...
    _createClass(Dropdown, [{
      ...
      value: function _clearMenus(event) {
        ...
        for (var i = 0; i < toggles.length; i++) {
          ...
          if (!$(_parent).hasClass(ClassName.SHOW)) {
            continue;
          }

          if (event && (event.type === 'click' && /input|textarea/i.test(event.target.tagName) || event.type === 'focusin') && $.contains(_parent, event.target)) {
            continue;
          }
          ...
          $(_parent).removeClass(ClassName.SHOW).trigger($.Event(Event.HIDDEN, relatedTarget));
        }
      }
    }, {
